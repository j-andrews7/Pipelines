#!/usr/bin/env python3
"""
Given lists of unmerged amps and dels for a sample, gets the signal for all SEs in that sample that lie within
the amps/dels, outside them, and for those that are unchanged and spits this info out to multiple files. Also
yields a few summary stats that may be helpful.
...
def main(amps_file, dels_file, SE_signal, ovlp_perc):
    ...
    print("SE_IN_AMPS" + "\t" + "SE_IN_DELS" + "\t" + "MB_AMPS" + "\t" +
          "MB_DELS" + "\t" + "SEs/MB_AMP" + "\t" + "SEs/MB_DEL", file=stats_out)
    print(str(SE_amps_count) + "\t" + str(SE_dels_count) + "\t" + str(amp_len) + "\t" +
          str(del_len) + "\t" + str(SE_amps_MB) + "\t" + str(SE_dels_MB), file=stats_out)
