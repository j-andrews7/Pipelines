...
if ($file =~ /(\S+)_treat.bin/) { # regular expression that matches each filename. It's a built-in function of perl that the first string in parentheses in a regex can be referred to as $1 (second thing in parenthesis is $2 etc..)
