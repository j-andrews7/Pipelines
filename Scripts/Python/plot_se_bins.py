...
size mayhelp the aesthetics of the image. The row labels are left off by default, but they can be
...
def main(input_file):
    ...
    bounds = [-5, -3, -1.5, 0, 1.5, 3, 5]
    norm = mpl.colors.BoundaryNorm(bounds, cmap.N)
    # Create the plot without y axis labels.
    heatmap = sns.heatmap(df, cbar=True, cmap=cmap, norm=norm, yticklabels=False)
